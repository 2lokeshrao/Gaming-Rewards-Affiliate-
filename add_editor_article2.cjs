...
if (!code.includes('AffiliateLinksEditor')) {
  code = code.replace(/import \{ Plus, Trash2, Edit3, Save, Search, RefreshCw, Loader \} from 'lucide-react';/, "import { Plus, Trash2, Edit3, Save, Search, RefreshCw, Loader } from 'lucide-react';\nimport { AffiliateLinksEditor } from './AffiliateLinksEditor';");
  
  const target = `<div className="md:col-span-2" data-color-mode="dark">`;
  const replacement = `
