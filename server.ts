...
async function setDoc(collection: string, docId: string, data: any) {
  try {
    ...
    await pool.query(
      'INSERT INTO app_data (collection, doc_id, data) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data)',
      [collection, docId, jsonData]
    );
  } catch (e) { logger.error('MySQL write error:', e); }
}

async function getCollection(collection: string) {
  try {
    const [rows] = await pool.query('SELECT data FROM app_data WHERE collection = ?', [collection]);
    return (rows as any[]).map(row => JSON.parse(row.data));
  } catch (e) { 
    logger.error('MySQL read error:', e); 
    return []; 
  }
}
...
async function getDoc(collection: string, docId: string) {
  try {
    const [rows] = await pool.query('SELECT data FROM app_data WHERE collection = ? AND doc_id = ?', [collection, docId]);
    const res = rows as any[];
    return res.length > 0 ? JSON.parse(res[0].data) : null;
  } catch (e) { 
    return null; 
  }
}
